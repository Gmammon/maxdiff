...
| 指标 | 含义 | 解读 |
|------|------|------|
| **效用值 (Utility)** | MLE 估计的原始效用值，正值表示偏好，负值表示不偏好 | 数值越大越被偏好，受零和约束，所有项之和为 0 |
| **标准误 (SE)** | 效用估计的标准差，反映估计精度 | 越小越精确；SE 过大说明数据不足 |
| **\|z\|** | 效用值 / 标准误，衡量统计显著性 | \|z\| > 1.96 为显著 (p<0.05)，\|z\| > 2.58 为高度显著 (p<0.01) |
| **分数 (Score)** | softmax 转换后的相对重要性：`exp(u) / Σexp(u) × 100` | 所有项总和为 100，可理解为"偏好份额" |

### 模型拟合相关

| 指标 | 含义 | 解读 |
|------|------|------|
| **对数似然 (Log-likelihood)** | 模型对数据的拟合程度 | 越接近 0 越好，绝对值取决于数据量 |
| **RLH (Root Likelihood)** | 预测正确选择的几何均值概率 | 范围 0~1，越高越好 |
| **基线 (Baseline)** | 随机猜测的 RLH = `1/(k×(k-1))` | k=4 时基线为 0.0833 |
| **RLH 倍率** | RLH / 基线，模型相对随机的优势 | > 2x 良好，> 5x 优秀 |
| **一致性 (Consistency)** | 受访者对重复任务的作答一致比例 | 建议筛选 ≥ 80% 的受访者 |
